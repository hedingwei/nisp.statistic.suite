...
//        cleanDir("/home/tmp/userflow");
        ...
        executeMapReduce(args[0], fl, args[1]);
//        grep(args[2]);
    }

    public void executeMapReduce(String namenodeip, String[] srcFolder, String date) throws IOException {
        /**
         * JobConf：map/reduce的job配置类，向hadoop框架描述map-reduce执行的工作
         * 构造方法：JobConf()、JobConf(Class exampleClass)、JobConf(Configuration
         * conf)等
         */
        String dstFolder = "/nisp/statistic/usertraffic_raw/" + date;
        ...
        config.set("thisDate", date);
        config.set("mapred.job.priority", JobPriority.LOW.toString());
        //        FileSystem fs = FileSystem.get(config);

        JobConf conf = new JobConf(config);
        // JobConf conf=new JobConf(WordCount.class);
        conf.setJobName("mr.ta.user.traffic_raw_" + date); // 设置一个用户定义的job名称
        conf.setOutputKeyClass(Text.class); // 为job的输出数据设置Key类
        conf.setOutputValueClass(Text.class); // 为job输出设置value类
        conf.setMapperClass(Map.class); // 为job设置Mapper类
//        conf.setCombinerClass(Reduce.class);
        conf.setReducerClass(Reduce.class); // 为job设置Reduce类
        conf.setJarByClass(Main12.class);
